...
class CAPQL(MOAgent, MOPolicy):
    ...
    def evaluate_for_front(self, eval_env: gymnasium.Env):
        num_eval_weights_for_front=15
        num_eval_episodes_for_front=6
        #eval_weights = equally_spaced_weights(self.reward_dim, n=num_eval_weights_for_front)
        eval_weights = [np.array([1., 0.])]
    ...
    def train(
        ...
        eval_weights = equally_spaced_weights(self.reward_dim, n=num_eval_weights_for_front)
        np.save(filename+"_weights.npy", np.array(eval_weights))
        ...
        for _ in range(1, total_timesteps + 1):
            ...
            if self.global_step >= self.learning_starts and (self.global_step % train_freq) == 0:
                self.update()

            if eval_env is not None and self.log and (self.global_step % (total_timesteps//20)) == 0:
                self.policy_eval(eval_env, weights=w, log=self.log, num_episodes=1)
            ...
            if terminated or truncated:
                ...
                wandb.log(
                    {
                        f"eval{idstr}/objective_1": vector_reward[0],
                        f"eval{idstr}/objective_2": vector_reward[1],
                        "global_step": self.global_step,
                    }
                )
                obs, info = self.env.reset()
            ...
            # Checkpoint
            self.save(filename=filename, save_replay_buffer=False)
